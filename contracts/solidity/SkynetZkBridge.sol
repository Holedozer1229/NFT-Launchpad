...
contract SkynetZkBridge is ReentrancyGuard, Ownable {
    error InvalidProof();
    error InvalidAmount();
    error InvalidChain();
    error NonceUsed();
    ...
    event ZkMintCompleted(bytes32 indexed proofHash, address indexed miner, uint256 tokenId, uint256 reward);
    ...
    function submitMintProof(
        ...
        bytes32 proofHash = keccak256(abi.encodePacked(
            msg.sender, nonce, uint8(sourceChain), amount, block.timestamp, block.number
        ));
        ...
        emit ZkProofSubmitted(proofHash, msg.sender, sourceChain, amount);
        return proofHash;
    }
    ...
    function guardianApproveMint(bytes32 proofHash) external onlyGuardian nonReentrant {
        ...
        if (proof.guardianSigs >= REQUIRED_SIGS) {
            ...
            uint256 reward;
            unchecked {
                reward = (proof.amount * multiplier) / 100;
            }
    ...
    function getMinerRewards(address miner) external view returns (
        uint256 totalMined_,
        uint256 totalBridged_,
    ...
    function getChainStats(MiningChain chain) external view returns (uint256) {
        return chainTotalMined[chain];
    }
