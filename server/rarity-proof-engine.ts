...
// ─── Auto-generate ZK certificate at mint time — fee waived (included in mint) ─
export async function generateRarityCertificateFree(nftId: number, userId: number) {
  const nft = await storage.getNft(nftId);
  if (!nft) throw new Error("NFT not found");

  // If already has a certificate, return it
  const existing = await storage.getRarityCertificateByNft(nftId, userId);
  if (existing) {
    return {
      certificate: existing,
      nft: { id: nft.id, title: nft.title, rarity: nft.rarity, chain: nft.chain, tokenId: nft.tokenId, owner: nft.owner },
      proof: {
        algorithm: "groth16-bn254",
        curve: "BN254",
        zkProofHash: existing.zkProofHash,
        verificationKeyHash: existing.verificationKeyHash,
        publicInputs: [nftId.toString(), existing.rarityScore.toString(), new Date(existing.createdAt ?? Date.now()).getTime().toString()],
      },
      rarityScore: existing.rarityScore,
      rarityPercentile: existing.rarityPercentile,
      phiBoost: existing.phiBoost,
      fee: 0,
      issuer: "SKYNT Protocol Oracle",
      issuedAt: existing.createdAt ?? new Date().toISOString(),
      autoGenerated: true,
    };
  }
  ...
  console.log(`[ZkRarityProof] Auto-generated certificate ${certificateId} for NFT#${nftId} (${nft.rarity}) — fee waived at mint`);
  ...
  return {
    ...
    proof: {
      ...
      zkProofHash,
      verificationKeyHash,
      publicInputs: [nftId.toString(), rarityScore.toString(), timestamp.toString()],
    },
    rarityScore,
    rarityPercentile,
    phiBoost: phiBoost.toFixed(4),
