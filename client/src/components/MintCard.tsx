...
import { Loader2, Rocket, Radio, Eye, Brain, Zap, Crown, Flame, Diamond, Gem, Link2, Fuel, ExternalLink, ShoppingBag, Server, Shield, CheckCircle2, Lock, ShieldCheck } from "lucide-react";
...
export function MintCard({ mission }: MintCardProps) {
  ...
  const [lastZkCert, setLastZkCert] = useState<{
    certificate: { certificateId: string; rarityScore: number; rarityPercentile: string; phiBoost: string; zkProofHash: string; status: string };
    rarityScore: number;
  ...
  const executeMint = async () => {
    ...
    try {
      ...
      if (nftResult.zkCertificate) {
        setLastZkCert(nftResult.zkCertificate);
      }
          ...
          <CardFooter className="pt-2 pb-6">
            {isConnected ? (
              <div className="w-full space-y-3">
                ...
                {lastZkCert && !isMinting && (
                  <div className="p-3 bg-black/40 border border-neon-green/30 rounded-sm space-y-2" data-testid="zk-cert-panel">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[8px] font-mono text-neon-green/60 bg-neon-green/10 border border-neon-green/20 rounded px-1 py-0.5">
                        {lastZkCert.autoGenerated ? "MINT-INCLUDED" : "GENERATED"}
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[9px] font-mono">
                      <div className="flex justify-between col-span-2">
                        ...
                        <span className="text-neon-green truncate max-w-[160px]">{lastZkCert.certificate.certificateId}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Score</span>
                        <span className="text-foreground">{lastZkCert.rarityScore}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Percentile</span>
                        <span className="text-foreground">{lastZkCert.rarityPercentile}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Φ Boost</span>
                        <span className="text-primary">{lastZkCert.phiBoost}×</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Algorithm</span>
                        <span className="text-foreground">{lastZkCert.proof.algorithm}</span>
                      </div>
                    </div>
                    <div className="pt-1.5 border-t border-white/5">
                      <div className="flex justify-between text-[8px] font-mono">
                        <span className="text-muted-foreground">ZK Proof Hash</span>
                        <span className="text-neon-green/70 truncate max-w-[150px]">{lastZkCert.proof.zkProofHash}</span>
                      </div>
                    </div>
                  </div>
                )}
