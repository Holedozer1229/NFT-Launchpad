...
import { Cpu, Eye, Database, Rocket, Users, CheckCircle2, AlertCircle, XCircle, Crown, Flame, Diamond, Gem, Link2, Fuel, ExternalLink, ShoppingBag, Loader2, Radio, ShieldCheck } from "lucide-react";
...
function StarshipMintModal({ flight, onClose }: StarshipMintModalProps) {
  ...
  const [mintedResult, setMintedResult] = useState<{
    ...
    zkCertificate: {
      certificate: { certificateId: string; rarityScore: number; rarityPercentile: string; phiBoost: string; zkProofHash: string };
      rarityScore: number;
      rarityPercentile: string;
      phiBoost: string;
      issuedAt: string;
      autoGenerated: boolean;
      proof: { zkProofHash: string; verificationKeyHash: string; algorithm: string };
    } | null;
  } | null>(null);
  ...
  const executeMint = async () => {
    ...
    try {
      ...
      setMintedResult({
        ...
        zkCertificate: result.zkCertificate || null,
      });
        ...
        <div className="space-y-5 pt-2">
          ...
          {mintedResult && !isMinting && (
            <div className="space-y-2">
              ...
              {mintedResult.zkCertificate && (
                <div className="p-3 bg-black/40 border border-neon-green/30 rounded-sm space-y-2" data-testid="zk-cert-modal-panel">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-[10px] font-heading uppercase tracking-wider text-neon-green">
                      <ShieldCheck className="w-3 h-3" /> ZK Rarity Certificate
                    </div>
                    <span className="text-[8px] font-mono text-neon-green/60 bg-neon-green/10 border border-neon-green/20 rounded px-1 py-0.5">
                      {mintedResult.zkCertificate.autoGenerated ? "MINT-INCLUDED" : "GENERATED"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[9px] font-mono">
                    <div className="flex justify-between col-span-2">
                      ...
                      <span className="text-neon-green truncate max-w-[160px]">{mintedResult.zkCertificate.certificate.certificateId}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Score</span>
                      <span className="text-foreground">{mintedResult.zkCertificate.rarityScore}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Percentile</span>
                      <span className="text-foreground">{mintedResult.zkCertificate.rarityPercentile}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Φ Boost</span>
                      <span className="text-primary">{mintedResult.zkCertificate.phiBoost}×</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Algorithm</span>
                      <span className="text-foreground">{mintedResult.zkCertificate.proof.algorithm}</span>
                    </div>
                  </div>
                  <div className="pt-1.5 border-t border-white/5">
                    <div className="flex justify-between text-[8px] font-mono">
                      <span className="text-muted-foreground">ZK Proof Hash</span>
                      <span className="text-neon-green/70 truncate max-w-[150px]">{mintedResult.zkCertificate.proof.zkProofHash}</span>
                    </div>
                  </div>
                </div>
              )}
